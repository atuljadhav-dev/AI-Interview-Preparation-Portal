...
const ReportPage = ({ id }) => {
    ...
    useEffect(() => {
        const fetchReport = async () => {
            try {
                const { data } = await api.get(`/ats/report/${id}`);
                setReport(data?.data);
  ...
  border border-purple-200 dark:border-gray-700 
  rounded-xl p-5 space-y-2 
   hover:scale-[1.02] transition duration-300 font-bold">
                                Alignment
                            </h2>
                            <p>
                                Coverage:{" "}
  ...
  rounded-xl p-5  space-y-2 
   hover:scale-[1.02] transition duration-300 font-bold">
                            ...
                            <p>
                                {report.atsReport.presence.contactDetails
                                    .email && "Email "}
                                {report.atsReport.presence.contactDetails
                                    .github && "GitHub "}
                                {report.atsReport.presence.contactDetails
